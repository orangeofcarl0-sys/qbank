...
The protocol is repository-bound after `repository.open`. `question.save` requires
...
Lifecycle methods are `initialize`, `repository.open`, `repository.status` and
...
`repository.open` creates one qbank `RepositorySnapshot`. `question.list` and
...
method changed during the monorepo migration. Advanced-management additions remain
