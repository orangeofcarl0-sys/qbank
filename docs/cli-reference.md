...
本文列出 qbank 0.2.0 的公共命令入口。参数、默认值和退出码以每个命令的 `--help` 以及
[0.2.0 兼容性基线](compatibility-0.2.0.md)为准；常用工作流见
[用户指南](user-guide.md)。
...
| `qbank preview` | 构建静态预览；`--serve` 只供交互使用 |
...
`asset edit`、`asset open` 和 `preview --serve` 不应由无人值守自动化静默启动。
